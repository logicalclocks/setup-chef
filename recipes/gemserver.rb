...
bash "start-gem-server" do
  ...
  code <<-EOH
      set -e
      if [ ! -d /opt/chefdk/embedded/lib/ruby/gems/#{node['setup']['gemserver']['version']} ] ; then
      ...
      nohup /opt/chefdk/embedded/bin/gem server --dir --port #{node['setup']['gemserver']['port']} /opt/chefdk/embedded/lib/ruby/gems/#{node['setup']['gemserver']['version']}/gems &
      EOH
end
